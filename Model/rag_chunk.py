...
# 定义 chunk 方法，使用 RecursiveCharacterTextSplitter 进行文本分块
def chunk(texts: list[str], chunk_size=200, chunk_overlap=0):
    """
    對文本進行分塊。
    
    參數：
        texts(list[str]): 原始文本
        chunk_size(int): 文本塊之大小
        chunk_overlap(int): 文本塊之間之重疊大小
...
def BM25_scores(qs, source, corpus_dict):
    ...
    tokenized_corpus = [list(jieba.cut_for_search(doc)) for doc in filtered_corpus]  # 将每篇文档进行分词
    bm25 = BM25Okapi(tokenized_corpus)  # 使用BM25算法建立检索模型
    tokenized_query = list(jieba.cut_for_search(qs))  # 将查询语句进行分词
    return bm25.get_scores(tokenized_query)


if __name__ == '__main__':

    mode = 'rerank'  # 'embed' 或 'rerank' BAAI/bge-reranker-v2-m3
    ...
    dim = 1024
    alpha = 0.5
    score_method = 'ws'  # 'rrf' 或 'ws'

    # 加载数据
    doc_index_ins = load_pdfs('reference/insurance')
    doc_index_fin = load_pdfs('reference/finance')
    ...
    for q_dict in tqdm(qs_ref['questions']):
        ...
        if mode == 'rerank':
            dense_scores = np.array(
                [rerank_model.compute_score([query[0], doc_index.docs[i]]) for i in candidates]).squeeze()
        elif mode == 'embed':
        ...
        if score_method == 'rrf':
            ...
            # 加权求和
            sparse_score = (sparse_score - sparse_score.min()) / (sparse_score.max() - sparse_score.min() + 1e-8)  # 归一化
            scores = alpha * dense_scores + (1 - alpha) * sparse_score

        best_match = doc_index.doc_ids[candidates[scores.argmax()]]
        answer_dict['answers'].append({"qid": q_dict['qid'], "retrieve": best_match})
